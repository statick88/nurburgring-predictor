...
/**
 * Fallback mock data for when race-data-service is not available
 */
...
export default function RaceLeaderboard({ entrants }: RaceLeaderboardProps) {
  ...
  // Use getRaceData() from service, fallback to provided entrants or mock
  ...
  return (
    <div className="bg-racing-darker rounded-lg border border-racing-light/10 overflow-hidden">
      ...
      {filters.categories.length > 0 && (
        <div className="bg-racing-dark/50 border-b border-racing-light/10 px-4 py-2 flex items-center gap-2">
          <span className="text-xs text-racing-muted">{t('Filtros activos', 'Active filters')}:</span>
      ...
      <div className="divide-y divide-racing-light/10 max-h-[600px] overflow-y-auto">
        {sortedEntrants.length === 0 ? (
          <div className="px-4 py-8 text-center text-racing-muted">
            {t('No hay resultados para los filtros seleccionados', 'No results for selected filters')}
          </div>
        ) : (
